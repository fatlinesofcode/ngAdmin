...
class DB
     ...
     */
        ...
        public static function fetch($sql, $fields = null, $type=PDO::FETCH_ASSOC)
        {
            return DB::query($sql, $fields)->fetch($type);
        }
    /*
     * $rows = DB::fetchAll('select * from players where id = :id', array('id' => $data['id']));
     */
        public static function fetchAll($sql, $fields = null, $type=PDO::FETCH_ASSOC)
        {
            return DB::query($sql, $fields)->fetchAll($type);
        }
